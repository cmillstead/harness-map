...
```jsonc
{
  ...
  "headline": {
    ...
    "hook_commands_examined": 0, "hook_commands_total": 0
  },
  "always_loaded": {
  ...
  "enforcement": {
    "hooks": {
      ...
      "commands_total": 0, "commands_resolved": 0, "commands_no_script": 0, "commands_unparsed": 0
    },
  ...
  "phantom_refs": [{"source": "rel", "ref": "", "kind": "path|env_flag|external|slash_command|template", "resolved": false /* |null */, "evidence": "VERIFIED|INFERRED"}],
...
These compose-only additions live INSIDE existing (A) arrays rather than as new top-level keys: every `always_loaded.files[]` entry AND every `conditional_variants[]` entry gains `tier` (`"operator"|"project"`), every `duplication.pairs[]` entry gains `a_tier`/`b_tier` (`"operator"|"project"`) once the corpus runs cross-tier, and every `promotion_candidates[]` entry gains `tier` (`"operator"|"project"`) once `--project-root` is also given (TRK-023, additive — no `schema_version` bump).
...
- **`headline`** — the original eight-number rollup used as the diff unit between runs (see Note 3 below; `render_html.py`'s `HEADLINE_KEYS`/`build_trend_model` track exactly these eight, by name, for trend sparklines), plus two TRK-025 additive counts appended after them (`hook_commands_examined`, `hook_commands_total` — see the `enforcement.hooks` bullet below; not part of the eight-series trend, since they are a coverage denominator rather than a defect count). Every field is a plain count, computed from the other sections. Every count is NON-NEGATIVE by domain, and readers must treat an ABSENT key as *not measured* rather than as `0`: `render_html.py` renders a missing key as the same `—` the trend table uses (never `0`, never a `CLEAN`/`COMPLIANT`/`LEAN` band), and a negative value is banded as no-verdict neutral rather than banding clean. A sidecar that is an UNMEASURED ENVELOPE — a collector CRASH ENVELOPE (all-zero headline plus a `collector crashed: …` entry in `errors[]`) or a layout-profile-rejection envelope (all-zero headline plus a `layout profile rejected: …` entry in `errors[]`, written when a `--profile` argument fails validation, TRK-051 F3) — is not a measurement at all: it is excluded from the trend series AND from sidecar selection, so it can never be rendered as the current run — with `--out-dir` alone the renderer falls back to the newest *measured* sidecar and discloses the skip in the provenance footer, and with an explicit `--date` naming one it fails rather than substituting a date the operator did not ask for.
...
- **`enforcement.hooks.commands_total`/`.commands_resolved`/`.commands_no_script`/`.commands_unparsed`** (TRK-025, additive — no `schema_version` bump) — the coverage denominator behind `orphan_registration_count`/`orphan_script_count`: every hook `command` registered in `settings.json` lands in EXACTLY one of the three buckets, so `commands_total` always equals their sum. `commands_resolved` is a command `_script_from_command` derived a script path for (its later existence/inaccessibility is tracked separately in `registered`/`orphan_registrations`/`inaccessible`, per Note 3 — this bucket is about tokenization, not disk state). `commands_no_script` is a command that tokenizes fine but demonstrably references no script at all (e.g. inline shell emitting a terminal escape sequence, or an env-prefixed/interpreter form with no script-shaped token) — fully examined, zero orphan risk, and deliberately NEVER added to `blind_spots`. `commands_unparsed` is the only bucket that reduces coverage, and there are THREE routes into it: `shlex` could not tokenize the command; or an unrecognized command form still appears to reference a script (a `.py`/`.sh`-suffixed token, or a token naming a file that exists under the profile's hooks dir); or the command tokenizes fine, names no script, and carries no shell control syntax the collector counts — a bare opaque invocation, noted as `unsupported hook command form: <command>`. That third route predates TRK-056 but carries more traffic after it: per AMENDMENTS A62 the control-syntax scan is quote-aware, so an operator sitting inside quotes is no longer evidence a shell is interpreting the command, and such a command falls through to this route instead of being called `no_script`. All three stay in `blind_spots` so a low `commands_resolved + commands_no_script` relative to `commands_total` is visible rather than the previous silent drop.
...
- **`phantom_refs[].kind` = `template`** (S6b / D4, additive — no `schema_version` bump) — a POST-probe RECLASSIFICATION of rows that would otherwise have been emitted as `kind: "path"`. It is applied only after the filesystem probe has already failed, so a token that resolves is dropped exactly as before regardless of its shape — the classification can never lose a legitimate resolution. `template` means the token names a SHAPE rather than a file: an angle or brace placeholder (`<slug>`, `{session}`), a shell glob (`*`, `?`), or a `YYYY-MM-DD` date stencil. It carries **`resolved: null`, `evidence: "INFERRED"`**, never `false`/`VERIFIED`, for the same reason `slash_command` does: asserting `resolved: false` about a file literally named `<slug>` or `*` would be a confirmed negative about something that was never a path. **`template` applies to PATH-SHAPED tokens only** — classification runs after ref detection, and a backticked token is detected as a reference only when it contains `/` or ends in a `[\w./~-]`-composed `.md`/`.py`/`.sh`/`.json`. A *bare* `<slug>.md`, `{session}.md` or `*.md` is therefore not detected as a reference at all: it produces no row, so there is nothing to classify and nothing is claimed about it. This limitation is recorded in `blind_spots`. A trailing line citation (`path.md:12-19`) is stripped from the PROBE TARGET only — the reported `ref` keeps the operator's original citation — and the post-strip probe uses the same tri-state `_safe_exists` as every other probe, then requires `is_file()` so a directory can never absorb an extension-bearing token. **Line-range validity is never checked**: a citation whose file exists drops out of the table even if the line range is stale, and that limitation is recorded in `blind_spots`.
  - *A sibling `refspec` kind is specified in S6 §7.2 and is NOT shipped: separating git refspecs from file paths lexically is undecidable, and the live corpus contains no such rows. Deferred to S6c, which will design it against real instances. A token like `origin/main` is therefore reported as `path` — what was actually probed — rather than dismissed as a git object.*
- **`promotion_candidates`** — prose in an instruction file that reads like a hard rule (`NEVER`, `ALWAYS`, `must`, a numeric cap, a required-file assertion) but has no corresponding hook enforcing it. `hook_covered` is `true` only when the collector matched it to an entry in `enforcement.hooks.registered`. **In `--compose` mode, when `--project-root` is also given** (TRK-023, additive — no `schema_version` bump), this array additionally covers the project tier and every row — operator or project — carries `tier: "operator"|"project"`; a row from a non-compose run, or a compose run with no `--project-root`, carries no `tier` key at all. See `collection_scope.hygiene_tiers` below for whether a given run's project-tier scan actually completed.
...
- **`collection_scope`** (S6c, additive — no `schema_version` bump) — the run's IDENTITY: `root` (`--root`, absolute), `project_root` (`--project-root`, absolute, or `null` when the collector was never given one), and `compose` (whether `--compose` was set). Two adjacent trend points whose `collection_scope` differs in ANY field are NOT COMPARABLE — the field exists so a consumer can make that call rather than guess from `headline` alone. A `null` `project_root` is a DISTINCT scope from any path, never "same as whatever ran last." Absent on a legacy sidecar predating S6c means UNKNOWN, and UNKNOWN adjacent to anything — including another UNKNOWN — is not comparable either.
  - **`collection_scope.hygiene_tiers`** (TRK-023, additive — no `schema_version` bump) — present only when `--compose` is set; a non-compose run OMITS the field entirely rather than emitting `["operator"]`. Which tiers the per-file hygiene analyses actually scanned, as exactly one of three fixed literals: `["operator"]` (a compose run given no `--project-root`, so no project tier was scanned at all), `["operator","project"]` (the project tier was scanned and the scan completed), or `["operator","project:partial"]` (the project tier was attempted but did not finish — a further `blind_spots` entry then names what was missed). `project:partial` is its own value on purpose, never collapsed into either neighbor: doing so would let two runs that failed in different ways carry an identical scope and compare, letting a partial scan chart a false improvement against a run that never attempted the project tier at all. Its presence is also *why* `promotion_candidate_count`'s `metric_definitions` version did NOT bump for this change — the metric's meaning is unchanged (still prose that reads like a rule), only compose mode's input corpus widens, and that widening is declared through `hygiene_tiers` rather than through a definition-version bump; a reader finding an unchanged version beside a changed count can learn here that the refusal to compare comes from `collection_scope`, not from a missed version bump. `render_html.py`'s `_scope_readable` validates `hygiene_tiers`, when PRESENT, to exactly these three literals — tolerates its total ABSENCE (an old sidecar predating TRK-023 keeps comparing on `root`/`project_root`/`compose` alone), and does not reject any OTHER unknown key `collection_scope` may carry, since `scope_comparable`'s own contract already covers "a field this module does not yet name."
...
- **`blind_spots`** — free-text notes on categories of content the collector structurally cannot see (e.g., runtime-only MCP server instructions, plugin-marketplace content not vendored locally). In `--compose` mode one entry discloses the per-file hygiene coverage split (TRK-023, additive — no `schema_version` bump): five analyses — `flag_long_instructions` (`instruction_length_flags`), `check_phantom_refs` (`phantom_refs`), `detect_test_coverage` (`test_coverage`), `_hooks_body_corpus` (the hooks-body duplication corpus), and `_staleness_corpus` (staleness) — scan the **operator tier only**, so a "0 project length flags" reading still means "not scanned," not "clean," for those five. One analysis, `collect_promotion_candidates` → `promotion_candidates`, can cover the project tier, and does so whenever `--compose` is set AND `--project-root` is given — including a PARTIAL scan's readable rows, which are still emitted with `tier: "project"`, never withheld. Read `collection_scope.hygiene_tiers` for what a given run actually scanned. Even when complete, that scan covers only the repo-root `CLAUDE.md`/`CLAUDE.local.md` and the `.claude/` rules, agents, commands and skills surfaces — **nested `CLAUDE.md` files are never scanned**. `check_phantom_refs` is deliberately NOT wired to the project corpus: it decides whether a reference is broken by asking the filesystem whether a file exists, which is an existence oracle when fed an untrusted, attacker-influenced path, so project-tier phantom-ref hygiene is cut on a security-design ground (TRK-023 plan header) rather than merely deferred. A STANDING, unconditional entry (TRK-082 T4, spec AMENDMENTS A59 requirement 4) discloses that a glob pattern with a wildcard in the *directory* position — the `skills/*/...` family (SKILL.md, phases, prompts, agents, rules, and hook-test globs) — has no single directory to probe for listability, so a locked intermediate `skills/<name>/` directory is indistinguishable from a genuinely empty one and its matches drop out with no per-pattern disclosure. This is narrower than the general glob surface: a single-directory glob's own unlistable target IS named (TRK-082 T1–T3), via the same OS-level `reason` shape as `inaccessible`, recorded to `blind_spots` or `errors` depending on the call site's existing channel.
