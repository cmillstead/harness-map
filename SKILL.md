...
Step A — Run the collector with the Bash tool, writing the sidecar DIRECTLY from the collector. Choose a report directory OUTSIDE `--root`, bind it to a single shell variable so every step stays aligned, create it, then run — all in ONE Bash call (shell variables do not persist across separate Bash calls): `OUT_DIR="$HOME/harness-map-reports" && mkdir -p "$OUT_DIR" && python3 ~/.claude/skills/harness-map/collector.py --root ~/.claude --project-root ~/.claude --out "$OUT_DIR/harness-map-$(date +%F).json"`. `OUT_DIR` defaults to `$HOME/harness-map-reports/`; the `mkdir -p "$OUT_DIR"` MUST run first — the collector creates the sidecar in that directory via a contained atomic write and will skip the sidecar (breaking the next run's diff) if it is absent. The `$(date +%F)` token produces the `YYYY-MM-DD` filename the D7 diff selection depends on. Remember the `OUT_DIR` value you used — Step B and the D7 diff reuse the SAME directory, and the D7 diff and trend view only accrue history when that SAME `OUT_DIR` is reused ACROSS RUNS too, so prefer this stable `$HOME` default over a cwd-relative one. Pin `--project-root` to the harness itself so the D7 diff is cwd-stable — `--project-root` defaults to the invoking shell's cwd, and an unpinned run from a product repo vs from `~/.claude` would fabricate headline deltas that are cwd artifacts, not real drift. It is a plain `python3` script — NO model needed to execute it. It emits JSON to stdout AND writes the sidecar; it reads only. The sidecar is the collector's output BYTE-FOR-BYTE — NEVER have the model re-serialize the JSON (a re-render can drift and would break the diff-vs-previous rule below).

Step B — Synthesis (this is the skill's model work): consume the collector JSON from stdout, read `~/.claude/skills/harness-map/report-template.md` AND `~/.claude/skills/harness-map/schema.md` by ABSOLUTE path with the Read tool (never cwd-relative), then write the report with the Write tool to the SAME report directory (`OUT_DIR`) as the sidecar: `$OUT_DIR/harness-map-<YYYY-MM-DD>.md` (matching the sidecar's date). Also write `$OUT_DIR/harness-synthesis-<YYYY-MM-DD>.json` (same date), a JSON file shaped `{"schema_version":1,"civc":[...all 36 verb×surface cells...],"drag_candidates":[...],"trend_basis":[...]}` conforming to schema.md section "(C) Synthesis sidecar file contract". The report `.md`, the collector-written sidecar `.json`, and the synthesis sidecar `.json` are the ONLY three outputs, all in `OUT_DIR` — NEVER write inside `~/.claude`.
...
Composed semantics are keyed per surface, not one global rule (live-verified 2026-08-07 against real `claude -p` sessions, CC 2.1.224 — all six surfaces matched; AMENDMENTS A63). **UNION surfaces** (CLAUDE.md, rules, hooks) — both tiers load; every project entry is an "add," never shadowed. **SHADOW surfaces** (skills, commands, agents) — a name collision has exactly one winner: skills/commands are **operator-shadows-project** (operator wins), agents are **project-shadows-user** (project wins) — this asymmetry is intentional, not a bug. A project skill/command that loses its collision is a **dark project skill**: defined in the repo but never runs while the operator's version is present.
...
Known v1 limitation (disclosed, not silent): in `--compose`, ONE per-file hygiene analysis — `promotion_candidates` (via `collect_promotion_candidates`) — now covers the project tier whenever `--project-root` is given, including a PARTIAL scan's readable rows (never withheld); read `collection_scope.hygiene_tiers` (`operator` / `project` / `project:partial`) to tell which case a given run was, and note that even a complete scan reaches only the repo-root CLAUDE.md/CLAUDE.local.md plus `.claude/{rules,agents,commands,skills}` — nested CLAUDE.md files are never scanned. The other five — `instruction_length_flags`, staleness, `phantom_refs` (deferred on a security-design ground: answering existence questions about attacker-influenced paths would make it an oracle), `test_coverage`, hooks-body duplication — still scan the OPERATOR tier only, and a `blind_spots` entry names each explicitly. Node composition, shadow resolution, settings/hooks/MCP merge, containment, and tier display DO cover both tiers.
...
Glob `$OUT_DIR/harness-map-*.json` (the same report directory `OUT_DIR` used in Step A), sort by the `YYYY-MM-DD` in the filename, take the most recent sidecar strictly BEFORE today's date that is a MEASURED run — skip any crash-envelope sidecar (same exclusion as the Hygiene view's Trend card below) and continue to the next-most-recent. Diff the headline numbers (always-loaded words/tokens, file count, dup-pair count, length-flag count, orphan counts) current-vs-prior. If no prior sidecar exists, state exactly: "First run — no prior map (baseline)."; if every prior sidecar is a crash envelope, state exactly: "No comparison baseline available — every prior run crashed."

The Hygiene view's Trend card — its first card, titled `Trend — direction over the measured history (N metrics)` — also grows a per-metric sparkline (last 10 points) once that metric's series has 3+ MEASURED points — a crashed collector run contributes none, and a sidecar missing that headline key contributes none to THAT metric, so sidecar count alone is no longer the gate — another reason to keep reusing that one stable `OUT_DIR` across runs.

Each row resolves to one of seven verdict words — `not measured` / `not comparable` / `no direction` / `unchanged across N` / `net unchanged` / `improving` / `worsening` — evaluated in that strict order; a confounded series (window not comparable on scope, definition version, or quality state) is FLAGGED `not comparable`, never given a direction verdict. For the `.md` report's section 6.1, fill each `{verdict}` cell by applying report-template.md's `<!-- TREND_VERDICT_TABLE -->` block to the matching series EXACTLY as Section 1 applies the Fixed band thresholds table (see Headline Bands above) — a lookup, never a judgment, NEVER invent a verdict per-run.

## `--check` — Regression Gate (M10)

`python3 collector.py --check OUT_DIR` (same D7 skip-crash-envelope selection above) compares this run's headline + latest CIVC synthesis against the most recent prior MEASURED run in `OUT_DIR` and prints findings instead of the JSON document. Exit **0** no regression (or no baseline yet — first run / every prior crashed), **1** a `REGRESSION:`-prefixed finding, **2** a collection/comparison error. Useful as a SessionStart hook gate (wiring the hook itself is out of scope here). A metric whose `metric_definitions` version differs between the two runs is skipped and named on a `notice:` line — a detector change is not a regression. The CIVC half is best-effort: when it cannot run — an unreadable out-dir, an unreadable or non-document synthesis sidecar, or a `civc` field present but not a list — the gate names it on a `notice:` line instead of staying silent; notices never change the exit code, and fewer than two synthesis sidecars existing yet (the ordinary early-life state) stays silent. A sidecar or synthesis filename whose date is structurally well-formed but not a real calendar date (e.g. `2026-02-31`) is never selected, here or by the renderer's `find_sidecars`.
