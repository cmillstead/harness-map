...
- SKILL.md hard cap **200** lines, working cap **195**, budgeted additions **≤26** (SPEC_3 §5) — contracts, changed only by spec amendment. Size: `wc -l SKILL.md`. Measured 2026-08-04: **106** (80 at the 2026-07-18 snapshot, 99 pre-S6a, 101 pre-M11; AMENDMENTS A12). 106 is inside every cap on either reading of the ≤26 budget's base; if that distinction ever becomes load-bearing, resolve it against SPEC_3 §5, not against this line.
- Description rewrite target **≤70** words (from the verified 88), all 4 positive + 2 negative triggers preserved (SPEC_3 §4).
- Test baseline: `python3 -m pytest -q` from the skill dir. Measured 2026-08-04 (`main` @ `c27b39d`, macOS): **952 passed / 3 skipped** in a plain environment; **954 passed / 1 skipped** with `HARNESS_MAP_REAL_SAMPLE` set; **955 items** either way (353 at the 2026-07-18 snapshot, 520 at S1, 765 at S6a, 851 at S6b — AMENDMENTS A10, A28). Count only goes up; existing tests untouched.
- **Two measuring traps.** (1) A PIPED `--collect-only` returns a wrapper line reading `Pytest: No tests collected`, which is not the real result — redirect to a file and read the file. (2) `HARNESS_MAP_REAL_SAMPLE` takes a **path to a sidecar whose filename matches `harness-map-YYYY-MM-DD.json`** (the discovery regex in `render_html.py::find_sidecars`) — not a boolean, and not any filename. A misnamed or absent file leaves the two real-data smoke tests skipped while appearing to enable them, so a test figure quoted without naming this variable's state is unreproducible.
...
- **When a measured figure above is wrong, correct it in the same change.** That requires a plan under `docs/plans/` declaring `instruction_files: CLAUDE.md` — `hooks/write-guard.py` blocks the edit otherwise, and routing through the Agent tool alone does NOT satisfy it.
...
**`SPEC_DIR/MILESTONE.md` is authoritative — read it, and prefer it over this summary, which drifts.** As of 2026-08-04: Stages 1, 2, 6a and 6b have shipped, and the scaffold (`check.sh`, `mypy.ini`) has existed since S1.M0 on 2026-07-24 — source verification completed 2026-07-18. **S7 is NOT complete:** TRK-019 + TRK-020 merged 2026-08-03 (PR #5, `06c6f2a`) and TRK-018 / M11 layout profiles merged 2026-08-04 (PR #6, `c27b39d`), but **TRK-021** (UI polish, blocked on operator scoping) and **M10** (`--check`, SPEC_7 §1) are both still open — so Stage 5 is not closed either.
